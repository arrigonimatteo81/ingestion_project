...
class PartitionBoundsResolver:
    ...
    def resolve(expression: str, query: str, db: DbConcrete) -> tuple[int, int]:
        ...
        sql = f"SELECT coalesce(MIN({NAME_OF_PARTITIONING_COLUMN}),0) as minimo, coalesce(MAX({NAME_OF_PARTITIONING_COLUMN}),99999) as massimo FROM ({query_for_partitioning}) t"
        db.connect()
        res = db.fetch_all(sql)[0]
        db.close()
        return res.get('minimo'), res.get('massimo')